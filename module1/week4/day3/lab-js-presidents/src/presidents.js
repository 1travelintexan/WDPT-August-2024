...
const firstFivePres = presidents.slice(0, 5);
// Iteration 1 | Names of All Presidents - `map()`
function getNames(presidentArr) {
  return presidentArr.map((onePres) => onePres.name);
}
...
function countYearsInOffice(pizza) {
  return pizza.reduce((acc, curr) => {
    if (curr.leftOffice) {
...
function sortPresidentsByBirthYear(pizza) {
  pizza.sort((curr, next) => curr.birthYear - next.birthYear);
  return pizza;
}
